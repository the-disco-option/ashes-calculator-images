  ...
  function (
    ...
    function _render(callback) {
      ...
      function attributeElement(data) {
        ...
        if (data.tag == 'svg') {
          ...
          if (options.width != null || options.height != null) {
            ...
            if (!options.fit) {
              element.attr(
                'viewBox',
                `0 0 ${(width * 3) / 4 / options.scale} ${(height * 3) / 4 / options.scale}`
              )
          ...
          if (
            ...
            element.attr(
              'viewBox',
              `0 0 ${width / options.scale} ${height / options.scale}`
            )
          ...
          elementTransition
            ...
            .on('end', function (d) {
              d3__namespace
                ...
                .attr(
                  'style',
                  (d && d.attributes && d.attributes.style) || null
                )
            })
        }
        ...
        if (moveThisPolygon) {
          var edgePath = d3__namespace.select(
    ...
    function logEvents(enable) {
      ...
      var maxEventTypeLength = Math.max(
        ...eventTypes.map((eventType) => eventType.length)
      )
    ...
    function Graphviz(selection, options) {
      ...
      if (useWorker || useSharedWorker) {
        ...
        var vizScript = scripts.filter(function () {
          return (
            d3__namespace.select(this).attr('type') == 'javascript/worker' ||
            (d3__namespace.select(this).attr('src') &&
              d3__namespace
                .select(this)
                .attr('src')
                .match(/.*\/@hpcc-js\/wasm/))
      ...
      if (useSharedWorker) {
        const url =
          ...
          btoa(
            workerCodeBody.toString() +
              '(' +
              sharedWorkerCode.toString() +
              ')()'
          )
        ...
        var blob = new Blob([
          workerCodeBody.toString() + '(' + workerCode.toString() + ')()',
        ])

...
function getBreakdown(item, totals) {
  ...
  for (let recipe of item.recipes) {
    ...
    for (let ing of recipe.getIngredients()) {
      ...
      rows.push(
        new BreakdownRow(ing.item, recipe, rate, building, count, null, false)
      )
      found = true
    }
  }
  ...
  for (let [recipe, rate] of totals.consumers.get(item)) {
    if (recipe.isReal()) {
      ...
      rows.push(
        new BreakdownRow(item, recipe, rate, building, count, percentStr, found)
      )
      found = false
    }
  }
...
class BeaconInput {
  ...
  checked() {
    return (
      this.module === this.cell.row.moduleSpec.beaconModules[this.cell.index]
    )
  }
  choose() {
    let toUpdate = [this.cell.index]
...
export function displayItems(spec, totals) {
  ...
  let row = rowGroup
    ...
    .join((enter) => {
      ...
      let beaconCountSpan = beaconCell
        .append('span')
  ...
  itemRow
    ...
    .text((d) =>
      spec.format.alignRate(
        totals.surplus.has(d.item) ? totals.surplus.get(d.item) : zero
      )
    )
  let beltRow = itemRow.filter((d) => d.item.phase === 'solid')
  let beltIcon = beltRow
    .selectAll('td.belt-icon')
  ...
  beltRow
    ...
    .text((d) =>
      spec.format.alignCount(spec.getBeltCount(totals.items.get(d.item)))
    )
  let pipeRow = itemRow.filter((d) => d.item.phase === 'fluid')
  let pipeIcon = pipeRow
    .selectAll('td.belt-icon')
  ...
  buildingRow.selectAll('tt.building-count').text((d) => d.building.name)
  ...
  moduleDropdown(
    moduleRow.selectAll('span.beacon-container'),
    (d) => d.beaconModules
  )
  moduleRow
    .selectAll('span.beacon-count input')

...
export function renderBoxGraph({ nodes, links }, ignore, callback) {
  ...
  for (let node of nodes) {
    ...
    let fill = d3
      .color(colorList[recipeColors.get(node.recipe) % colorList.length])
      .darker()
      .hex()
    let stroke = colorList[recipeColors.get(node.recipe) % colorList.length]
    dot.push(
      `    "${node.name}" [label="" shape=rect fixedsize=true width="${width / dpi}" height="${height / dpi}"];\n`
    )
  }
  ...
  for (let [i, link] of links.entries()) {
    ...
    let fill = d3
      .color(colorList[itemColors.get(link.item) % colorList.length])
      .darker()
      .hex()
    ...
    link.label = label
    dot.push(
      `    "${link.source.name}" -> "${link.target.name}" [label="MNII${s}" color="${stroke}" fillcolor="${fill}" penwidth=3];\n`
    )
  }
  dot.push('};\n')
  text.remove()
  ...
  div.graphviz(graphvizConfig).renderDot(dotText, () => {
    let svg = div.select('svg').attr('id', 'graph').classed('sankey', false)
    let tab = d3.select('#graph_tab')
    let style = tab.style('display')
    tab.style('display', 'block')
    let rects = svg.selectAll('.node').each(function () {
    ...
    let edges = svg.selectAll('.edge').each(function () {
      let selector = d3.select(this)
    ...
    edges.selectAll('path, polygon').classed('highlighter', true)
    edges
      .filter((d) => d.fuel)
      .selectAll('path')
      .classed('fuel', true)
